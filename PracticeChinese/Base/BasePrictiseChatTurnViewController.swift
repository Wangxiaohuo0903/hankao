//
//  BasePrictiseChatTurnViewController.swift
//  PracticeChinese
//
...
class BasePrictiseChatTurnViewController: UIViewController,SettingViewDelegate {
    ...
    var shouldShowWithTransform = false {
        didSet {
            if shouldShowWithTransform {
                ...
                let footerView  = UIView(frame: CGRect(x: 0, y: 0, width: ScreenUtils.width - 18, height: footerHeight))
                footerView.backgroundColor = UIColor.white
                tableView.tableFooterView = footerView
                
                let headerView  = UIView(frame: CGRect(x: 0, y: 0, width: ScreenUtils.width - 18, height: 95))
                ...
                self.tableView.transform = CGAffineTransform(scaleX: 1, y: 1)
                let footerView  = UIView(frame: CGRect(x: 0, y: 0, width: ScreenUtils.width - 18, height: 95))
                footerView.backgroundColor = UIColor.white
                tableView.tableFooterView = footerView
                
                let headerView  = UIView(frame: CGRect(x: 0, y: 0, width: ScreenUtils.width - 18, height: 10))
                headerView.backgroundColor = UIColor.white
                tableView.tableHeaderView = headerView
            }
        ...
        for msg in self.msgList {
            ...
            case ChatMessageType.audiotext:
                if msg.position == .right {
                    ...
                    let chineseandpinyinLabel = SpeakTokensView(frame: CGRect(x: 30, y: 50, width: cardMaxWidth, height: 150), tokens: textArray, chineseSize: chineseSize, pinyinSize: pinyinSize, style: .chineseandpinyin, changeAble: true,showIpa:false,scope: self.Scope,cColor:UIColor.black,pColor:UIColor.black,scoreRight:false)
    ...
    override func viewDidLoad() {
        ...
        // add dialogue table
        tableView = UITableView(frame: CGRect(x: 9, y: self.ch_getStatusNavigationHeight(), width: ScreenUtils.width - 18, height: tableHeight), style: .grouped)
        ...
        self.gradientColor(gradientView: recordViewBg, frame: CGRect(x: 9, y: 0, width: ScreenUtils.width, height: 125), upTodown: true)
...
extension BasePrictiseChatTurnViewController: UITableViewDataSource, UITableViewDelegate, LCVoiceButtonDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case ChatMessageType.audiotext:
            if msg.position == .left {
                ...
                if (self.msgList[0].type == .audiotext) && (self.msgList[0].position == .left) && self.bubbleCanPlayAudio {
    ...
    func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        ...
        let tableHeight = ScreenUtils.height - CGFloat(bottomHeight) - CGFloat(self.ch_getStatusNavigationHeight())
        var allCellHeight = CGFloat()
        var bigger = false
        for height in cellHeight {
            allCellHeight += height
