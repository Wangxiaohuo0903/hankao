...
- (void)drawCalibration {
    ...
    circleRadius = self.width_s/2 - bottomWidth/2 - _edgeSpace;
    //画背景
    [self drawBottom];
    //画刻度
    [self drawScale];
...
- (void)drawScale {
    ...
    for (NSInteger i = 0; i < totalScale + 1; i ++) {
        ...
        if (i%_smallScaleNum == 0) {
            ...
            UIBezierPath *majorPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.width_s/2, self.height_s/2)
                                                                     radius:circleRadius
            ...
            UIBezierPath *smallPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.width_s/2, self.height_s/2) radius:circleRadius
                                                                 startAngle:DEGREES_TO_RADIANS(startAngle)
                                                                   endAngle:DEGREES_TO_RADIANS(endAngle)
                                                                  clockwise:YES];
...
//    }
    ...
    for (NSInteger i = 0; i < textArray.count; i ++) {
        ...
        CGPoint majorCenter = CGPointMake(self.width_s/2+textRadius*cosf(angle), self.height_s/2+textRadius*sinf(angle));
        ;
        ...
        if (_textType == MLMCalibrationViewCustom) {
            //使用中心点
            scaleLable.center = CGPointMake(majorCenter.x - scaleLable.height_s/2*cosf(angle), majorCenter.y - scaleLable.height_s/2*sinf(angle));
