    ...
    if (yStep == 0.0) {
        ...
        while (num > 0) {
            CGRect labelFrame = CGRectMake(0.0,
                    (NSInteger) (_chartCavanHeight - index * yStepHeight),
                    (CGFloat) ((NSInteger) _chartMarginLeft),
...
- (void)setYLabels:(NSArray *)yLabels withHeight:(CGFloat)height {
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < yLabels.count; index++) {
            ...
            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, y, (CGFloat) ((NSInteger) _chartMarginLeft), (NSInteger) _yLabelHeight)];
            [label setTextAlignment:NSTextAlignmentRight];
...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width {
    ...
    if (_showLabel) {
        for (NSUInteger index = 0; index < xLabels.count; index++) {
            ...
            NSInteger y = (NSInteger) (15 + _chartCavanHeight);
...
- (void)prepareYLabelsWithData:(NSArray *)data {
    ...
    _yValueMin = (_yValueMin < _yBaseline) ? _yValueMin : _yBaseline;
    _yValueMax = (_yValueMax > _yBaseline) ? _yValueMax : _yBaseline;
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.showYGridLines) {
        ...
        //baseline
        CGFloat yBaseHeight = _chartCavanHeight * (_yValueMax - _yBaseline) / (_yValueMax - _yValueMin)  + _yLabelHeight / 2;
        CGPoint FirstCGPoint = CGPointMake(_chartMarginLeft + yAxisOffset, yBaseHeight) ;
        CGContextMoveToPoint(ctx, FirstCGPoint.x, FirstCGPoint.y) ;
        if (self.yBaseLinesColor) {
            CGContextSetStrokeColorWithColor(ctx, self.yBaseLinesColor.CGColor);
        } else {
            CGContextSetStrokeColorWithColor(ctx, [[UIColor colorWithRed:91.0f/255.0f
                                                                   green:157.0f/255.0f
                                                                    blue:255.0f/255.0f alpha:0.8] CGColor]);
        }

        CGContextSetLineDash(ctx, 0.0, NULL, 0);
        CGContextSetLineWidth(ctx, 1);

        CGPoint SecondCGPoint = CGPointMake(CGRectGetWidth(rect) - _chartMarginLeft + 5, FirstCGPoint.y) ;
        CGContextAddLineToPoint(ctx, SecondCGPoint.x, SecondCGPoint.y);
        CGContextStrokePath(ctx);
...
- (CGFloat)yValuePositionInLineChart:(CGFloat)y {
    ...
    return _chartCavanHeight - (innerGrade * _chartCavanHeight) - (_yLabelHeight) + _chartMarginTop;
}
