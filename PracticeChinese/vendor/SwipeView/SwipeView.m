...
@property (nonatomic, assign) CGFloat deltaN;
...
- (void)didScroll
{
    ...
    {
        //update item index
        CGFloat d = _deltaN>0? 0.40:-0.40;
        _currentItemIndex = [self clampedIndex:roundf(_scrollOffset - d)];
        //load views
        _lastUpdateOffset = _currentItemIndex;
        [self loadUnloadViews];
        //send index update event
        if (labs(_previousItemIndex - _currentItemIndex) > 2) {
