...
- (instancetype)init {
    CGRect frame = CGRectMake(0, 0, DEVICE_WIDTH, 40);
...
- (void)setupItems {
    ...
    _scrollView.contentSize = CGSizeMake(itemX + 100, CGRectGetHeight(_scrollView.frame));
...
- (void)selectSlideBarItemAtIndex:(NSInteger)index {
...
- (void)slideBarItemSelected:(FDSlideBarItem *)item {
    ...
    [self addAnimationWithSelectedItem:item];
    self.selectedItem = item;
    _callback([self.items indexOfObject:item],_itemsTitle[[self.items indexOfObject:item]]);
}
