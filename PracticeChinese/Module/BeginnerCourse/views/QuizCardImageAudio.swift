//
//  QuizCardSingleImageAudioText.swift
//  PracticeChinese
//
...
class QuizCardImageAudio: QuizCardSuper {
...
//        textLabel.text = "哪个选项和图片最匹配?"
        ...
        videoButton = CircularProgressView.init(frame: CGRect(x:6.5 , y:6.5, width: 15, height: 15), back: UIColor.hex(hex: "E8F0FD"), progressColor: UIColor.hex(hex: "AECFFF"), lineWidth: 2, audioURL: nil, targetObject:self)
        ...
        imageView.addSubview(videoButtonView)
        ...
        if(imageWidth/3*2 > quizViewHeight && imageWidth/16*9 < quizViewHeight){
            imageHeight = imageWidth/16*9
        }else if(imageWidth/16*9 > quizViewHeight){
            imageHeight = quizViewHeight
        }
    ...
    override func setData(quiz:QuizSample,voice:Dictionary<String,SentenceDetail>){
        ...
        if(voice[(quiz.Body?.AudioText)!]?.PictureUrl != nil || quiz.Body?.Url != nil){
            ...
            if(url != nil){
                imageView.sd_setImage(with: url, placeholderImage: UIImage.fromColor(color: UIColor.lightGray), options: .refreshCached) { (image, error, type, url) in
                }
            }
        }
        ...
        if let OptionCount = quiz.Options?.count {
            if OptionCount <= 0 {
                return
            }
            if let tokenCount = quiz.Options?[0].Tokens?.count {
                if tokenCount <= 0 {
                    return
                }
            }
